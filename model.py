...
class ModelConfig:
    ...
    def __init__(self,model_name,
        ...
        self.gfm_embedding = gfm_embedding
        self.gfm_dropout = gfm_dropout
        self.gfm_n_blocks = gfm_n_blocks
        self.gfm_d_hidden = gfm_d_hidden
        self.gfm_d_out = gfm_d_out
...
class MuAtMotifPositionGESF_2Labels(nn.Module):
    ...
    def __init__(self, config):
        super().__init__()
        print('MuAtMotifPositionGESF_2Labels')
        self.config = config 

        self.gfm_embedding = self.config.gfm_embedding
        self.use_cls_token = getattr(self.config, 'use_cls_token', False)  # Add CLS token option

        self.num_tokens, self.max_pool = config.motif_size, False

        self.token_embedding = nn.Sequential(
            nn.Embedding(config.motif_size, config.n_embd, padding_idx=0),
            nn.LayerNorm(config.n_embd, elementwise_affine=False)  # Normalization layer
        )
        self.position_embedding = nn.Sequential(
            nn.Embedding(config.position_size, config.n_embd, padding_idx=0),
            nn.LayerNorm(config.n_embd, elementwise_affine=False)  # Normalization layer
        )
        self.ges_embedding = nn.Sequential(
            nn.Embedding(config.ges_size + 1, 4, padding_idx=0),
            nn.LayerNorm(4, elementwise_affine=False)  # Normalization layer
        )


        # Add CLS token embedding if enabled
        if self.use_cls_token:
            self.cls_token = nn.Parameter(torch.zeros(1, 1,  config.n_embd + # config.n_embd + # 4 + 
                                         (self.config.gfm_d_out if self.gfm_embedding else 0)))
            nn.init.normal_(self.cls_token, std=0.02)


        if self.gfm_embedding:
            # self.projection = MLPBlock(
            #                            d_in=1024,
            #                            d_hidden=self.config.gfm_d_hidden,
            #                            d_out=self.config.gfm_d_out,
            #                            dropout=self.config.gfm_dropout,
            #                            n_blocks=self.config.gfm_n_blocks,
            #                            bias=False,
            #                            activation='swiglu',
            #                            pre_norm=True,
            #                            post_norm=False,
            ...
            #     nn.Linear(
            #         1024,
            #         self.config.expansion_factor * self.config.gfm_d_hidden * (2 if self.config.activation == "swiglu" else 1),
            #         bias=False,
            #     ),
            #     _get_activation_fn(self.config.activation),
            #     nn.Dropout(self.config.gfm_dropout),
            #     nn.Linear(self.config.expansion_factor * self.config.gfm_d_hidden, self.config.gfm_d_out, bias=False),
            # )

            # self.projection = nn.Sequential( # worked at old setting datas 
            #     # nn.RMSNorm(1024), 
            #     nn.Linear(1024, self.config.gfm_d_hidden, bias=False), 
            #     # nn.RMSNorm(self.config.gfm_d_hidden), 
            #     _get_activation_fn(self.config.activation),
            #     nn.Linear(
            #         self.config.gfm_d_hidden,
            #         self.config.expansion_factor * self.config.gfm_d_hidden * (2 if self.config.activation == "swiglu" else 1),
            #         bias=False,
            #     ),
            #     _get_activation_fn(self.config.activation),
            #     # nn.Dropout(self.config.gfm_dropout),
            #     nn.Linear(self.config.expansion_factor * self.config.gfm_d_hidden, self.config.gfm_d_out, bias=False),
            #     nn.RMSNorm(self.config.gfm_d_out), 
            # )

            self.projection = nn.Sequential( 
                
                # nn.LayerNorm(self.config.gfm_d_out* (2 if self.config.activation == "swiglu" else 1), elementwise_affine=False),  # Normalization layer
                # nn.LayerNorm(1024, elementwise_affine=False),  # Normalization layer
                nn.Linear(
                    1024,
                    # self.config.expansion_factor * self.config.gfm_d_hidden * (2 if self.config.activation == "swiglu" else 1),
                    self.config.gfm_d_out* (2 if self.config.activation == "swiglu" else 1)
                ),
                _get_activation_fn(self.config.activation),
                nn.LayerNorm(self.config.gfm_d_out, elementwise_affine=False),  # Normalization layer
                
                
            )

        self.d_model = int(config.n_embd + config.n_embd + 4 + (self.config.gfm_d_out if self.gfm_embedding else 0))
        self.d_model = int(config.n_embd + config.n_embd + (self.config.gfm_d_out if self.gfm_embedding else 0))
        self.d_model = int(config.n_embd + (self.config.gfm_d_out if self.gfm_embedding else 0))
        # self.d_model = int( (self.config.gfm_d_out if self.gfm_embedding else 0))
        tblocks = []
        for i in range(config.n_layer):
            seq_len = (config.mutation_sampling_size + 1) if self.use_cls_token else config.mutation_sampling_size
            tblocks.append(
                # TransformerBlock(emb=int(config.n_embd + config.n_embd + 4), heads=config.n_head, seq_length=config.mutation_sampling_size, mask=False, dropout=config.attn_pdrop))
                TransformerBlockPlus(emb=self.d_model, heads=config.n_head, seq_length=seq_len, mask=False, dropout=config.attn_pdrop))

        self.tblocks = nn.Sequential(*tblocks)
        
        self.to_joinfeatures = nn.Sequential(nn.Linear(self.d_model, 64),
                                       nn.ReLU())
    ...
    def forward(self, x, targets=None, vis=None,get_features=False):
        batch_size = x.size(0)
        triplettoken = x[:, 0, :].to(torch.long)
        postoken = x[:, 1, :].to(torch.long)
        # gestoken = x[:, 2, :].to(torch.long)
        ...
        if self.gfm_embedding:
            mut_gfm_embedding = x[:, 3:1027, :].transpose(-1,-2)
            ref_gfm_embedding = x[:, 1027:, :].transpose(-1,-2)
            # gfm_embedding = torch.cat((mut_gfm_embedding, ref_gfm_embedding), dim=-1)  # Concatenate along the last dimension
            gfm_embedding = mut_gfm_embedding - ref_gfm_embedding  # Subtract the reference from the mutated GFM embedding
            gfm_e = self.projection(gfm_embedding)
            # gfm_e = self.projection(gfm_embedding.transpose(-1,-2))
        
        if self.gfm_embedding:
            # x = gfm_e
            # x = torch.cat(( tokens, positions, gfm_e), axis=2)
            x = torch.cat(( positions, gfm_e), axis=2)
            # x = torch.cat((tokens, positions, ges, gfm_e), axis=2)
            # # print(f"Mean x: {x.mean()}, std x: {x.std()}")
            if vis:
                # print(f"Mean tokens: {tokens.mean()}, std tokens: {tokens.std()}")
                # print(f"Mean positions: {positions.mean()}, std positions: {positions.std()}")
                # print(f"Mean ges: {ges.mean()}, std ges: {ges.std()}")
                print(f"Mean gfm_e: {gfm_e.mean()}, std gfm_e: {gfm_e.std()}")
            # x = torch.cat((x, gfm_e), axis=2)

            # x = torch.cat((tokens, positions), axis=2)
            # x = x + gfm_e
            # x = torch.cat((x, ges), axis=2)
        else:
            # x = torch.cat((tokens, positions, ges), axis=2)
            x = torch.cat((tokens, positions), axis=2)
            # x = positions # tokens # positions # testing token only method
            if vis:
                # print(f"Mean tokens: {tokens.mean()}, std tokens: {tokens.std()}")
                print(f"Mean positions: {positions.mean()}, std positions: {positions.std()}")
                # print(f"Mean ges: {ges.mean()}, std ges: {ges.std()}")
        ...
        if self.use_cls_token:
            cls_tokens = self.cls_token.expand(batch_size, -1, -1)  # Expand for batch
            # print(x.shape, cls_tokens.shape)
            x = torch.cat([cls_tokens, x], dim=1)  # Prepend CLS token
        
        # x = self.do(x) # drop out on embedding, from MUAT implementation
        # # pdb.set_trace()
        ...
        if targets is not None:
            # pdb.set_trace()
            # targets are (B, 2) where first column is type and second column is subtype
            loss1 = F.cross_entropy(typeprobs.view(-1, typeprobs.size(-1)), targets[0].view(-1,1))
            loss2 = F.cross_entropy(subtypeprobs.view(-1, subtypeprobs.size(-1)), targets[1].view(-1,1))
            loss = loss1 + loss2
