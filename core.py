...
def load_weights(model, checkpoint):
    """
    Load appropriate weights into model based on available averaging method in checkpoint.
    Priority: EMA > SWA > raw weights
    """
    model_weights = checkpoint['weight']
    weight_info = model_weights.get('weight_averaging_info', {})
    print(checkpoint.keys())
    print(weight_info)
    if weight_info.get('averaged_weights_used', False):
        method = weight_info.get('averaging_method')
        if method == 'ema':
...
def main():
    ...
    get_GFM_embedding = args.useGFM
    use_CLS_token = args.useCLStoken
    ...
    if args.command == 'wgs' or args.command=='wes':
        ...
        if args.mutation_type is not None:
            raise ValueError('Please provide --mutation-type for prediction.')
            if args.command=='wgs':
                wgs_wes = 'wgs'
        ...
        if args.no_preprocess:
            print(f"Got --no-preprocess flag, skipping preprocessing. Using raw VCF files: {vcf_files}")
            predict_ready_files = vcf_files
            predict_ready_files = []
            for x in vcf_files:
                sample_path = os.path.join(tmp_dir, get_sample_name(x) + '.muat.tsv')
                if os.path.exists(sample_path):
                    predict_ready_files.append(sample_path)

            if get_GFM_embedding:
                embedding_files = []
                for x in vcf_files:
                    # print(get_sample_name(x))
                    sample_path = os.path.join(tmp_dir, get_sample_name(x) + '.merged.embedding.npz')
                    if os.path.exists(sample_path):
                        embedding_files.append(sample_path)
                # remove predict_ready_files that not in embedding_files
                predict_ready_files = [x for x in predict_ready_files if x.replace('.muat.tsv', '.merged.embedding.npz') in embedding_files]
                embedding_files = [x.replace('.muat.tsv', '.merged.embedding.npz') for x in predict_ready_files]

                pd_predict = pd.DataFrame({'prep_path': predict_ready_files, 'embedding_path': embedding_files})
            else:
                pd_predict = pd.DataFrame(predict_ready_files, columns=['prep_path'])
        elif args.hg19 is not None:
            if get_GFM_embedding:
                raise ValueError('Embedding is not supported for hg19 preprocessing. Please use hg38 or no preprocessing.')
            genome_reference_path_hg19 = resolve_path(args.hg19)
            ...
            if get_GFM_embedding:
                raise ValueError('Embedding is not supported for hg38 preprocessing. Please use hg19 or no preprocessing.')
            genome_reference_path_hg38 = resolve_path(args.hg38)
            ...
            raise ValueError('Please provide --hg19 or --hg38 for preprocessing.')
        print(pd_predict)
        target_handler = load_target_handler(checkpoint)

        dataloader_config = checkpoint['dataloader_config']
        test_dataloader = MuAtDataloader(pd_predict,dataloader_config,get_embedding=get_GFM_embedding, liftover=args.GFMliftover)
    ...
    if args.command == 'from-scratch':
        ...
        if arch != "MuAtMotifPositionGESF_2Labels" and get_GFM_embedding:
            raise ValueError('GFM embedding is not supported for this architecture. Please use MuAtMotifPositionGESF_2Labels.')
        ...
        model_config = ModelConfig(
            ...
            gfm_embedding=get_GFM_embedding,
            gfm_dropout=0.0,
            gfm_n_blocks=1,
            expansion_factor=4,
            activation="swiglu",
        ...
        trainer_config = TrainerConfig(
            ...
            num_workers=32,
            show_loss_interval=31,
            save_ckpt_dir=save_dir,
            gfm_embedding=get_GFM_embedding,
            use_ema=getattr(args, 'use_ema', True),
            ema_decay=getattr(args, 'ema_decay', 0.95),
            use_swa=getattr(args, 'use_swa', False),
            swa_start_epoch=getattr(args, 'swa_start_epoch', 5),
            use_CLS_token=use_CLS_token,
            target_handler=[]
        )
        ...
        train_dataloader = MuAtDataloader(train_split, train_dataloader_config, get_embedding=get_GFM_embedding)
        test_dataloader = MuAtDataloader(test_split, test_dataloader_config, get_embedding=get_GFM_embedding)
    ...
    if args.command == 'muat-wgs' or args.command=='muat-wes':
        ...
        for i_fold in range(len(check_pth)):
            ...
            # check those with empty value in postoken column in pd_predict  
            if pd_predict['prep_path'].isnull().values.any():
                raise ValueError('There are some samples with empty value in postoken column. Please check your input file.')

            target_handler = load_target_handler(checkpoint)

            dataloader_config = checkpoint['dataloader_config']
            test_dataloader = MuAtDataloader(pd_predict,dataloader_config, get_embedding=gfm_embedding)
