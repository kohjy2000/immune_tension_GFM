...
def get_main_args():
    ...
    wgs.add_argument('--useGFM', action='store_true', default=False,
                        help='Use GFM in the model')
    wgs.add_argument('--useCLStoken', action='store_true', default=False,
                        help='Use CLS token in the model')
    wgs.add_argument('--GFMliftover', action='store_true', default=False,
                        help='Use GFM_liftover in the dataloader')
    ...
    from_scratch.add_argument('--useGFM', action='store_true', default=False,
                        help='Use GFM in the model')
    from_scratch.add_argument('--useCLStoken', action='store_true', default=False,
                        help='Use CLS token in the model')
...
def read_reference(reffn, verbose=0, target_chr=None):
    R = {}
    chrom = None
    seq = []
    f = None
    ...
    try:
        if reffn.endswith('.gz'):
            if verbose:
                sys.stderr.write('Decompressing gzipped ref file...\n')
        ...
        for s in f:
            if s[0] == '>':
                ...
                if verbose:
                    sys.stderr.write('--read chr {} '.format(chrom))
                    sys.stderr.flush()
            else:
