...
def comp_base(b: str) -> str:
    return b.translate(_rc_table)
def get_context_seq(prev_buf, next_buf, ref_genome, context=2406, debug=False):
    """
    Efficiently extract mutation-aware raw DNA sequence from the reference genome using sparse edits.
    Does not copy the chromosome string except when generating the final output.

    Args:
        v (Variant): focal variant
        prev_buf, next_buf (list of Variant): surrounding variants
        ref_genome (dict): reference genome
        context (int): total context length in bases

    Returns:
        str or None: sequence with mutations applied, fixed length, or None if out-of-bounds
    """
    v = next_buf[0]
    chrom = v.chrom
    flank = context // 2
    ...
    # v.pos is 1-indexed
    base_start = v.pos - flank - 1
    base_end = v.pos + flank 

    if chrom not in ref_genome:
        if debug:
            return None, f"chrom {chrom} not found in reference genome"
        return None, None

    chrom_seq = ref_genome[chrom]
    if base_start < 0 or base_end >= len(chrom_seq):
        if debug:
            return None, f"base_start {base_start} or base_end {base_end} out of bounds for chromosome {chrom} of length {len(chrom_seq)}"
        return None, None

    ref_seq = chrom_seq[base_start:base_end]  # length == context

    # Calculate focal position relative to base_start
    focal_pos = v.pos - 1 - base_start  # This should be equal to flank

    edits = {}  # position (relative to base_start) -> str (edited base or insertion)
    
    if v.vtype == Variant.SNV:
        assert ref_genome[chrom][v.pos-1] == v.ref , f"reference base {ref_genome[chrom][v.pos-1]} does not match the variant reference base {v.ref} at position {v.pos}"
    elif v.vtype == Variant.DEL:
        assert ref_genome[chrom][v.pos-1] == v.ref[0] , f"reference base {ref_genome[chrom][v.pos-1]} does not match the variant reference base {v.ref} at position {v.pos}"
    ...
    for v2 in itertools.chain(prev_buf,  next_buf):
        ...
        if v2.vtype == Variant.SNV:
            assert len(v2.ref) == 1 and len(v2.alt) == 1, f"Invalid SNV variant: {v2}"
        elif v2.vtype == Variant.DEL:
            assert len(v2.ref) - len(v2.alt) >= 1, f"Invalid DEL variant: {v2}"
        elif v2.vtype == Variant.INS:
            assert len(v2.alt) - len(v2.ref) >= 1, f"Invalid INS variant: {v2}"

        if v2.chrom != chrom:
            continue

        rel_pos = v2.pos - 1 - base_start  # relative to start of our window
        if rel_pos < 0 or rel_pos >= context:
            continue
        ...
        if v2.vtype == Variant.SNV:
            edits[rel_pos] = v2.alt

        elif v2.vtype == Variant.DEL:
            for i in range(len(v2.ref)):
                if 0 <= rel_pos + i < context:
                    del_pos.add(rel_pos + i)

        elif v2.vtype == Variant.INS:
            ins_seq = v2.alt
            edits[rel_pos] = list(chrom_seq[base_start + rel_pos] + ins_seq)

    # Generate final sequence using reference, with edits applied
    seq = []
    focal_pos_in_seq = None
    curr_pos = 0

    # Build the edited sequence and track focal position
    for i in range(base_end - base_start):
        # Check if we've reached or passed the focal position
        if focal_pos_in_seq is None and i >= focal_pos:
            focal_pos_in_seq = curr_pos
        
        if i in del_pos:
            continue
        elif i in edits:
            if isinstance(edits[i], list):
                seq.extend(edits[i])
                curr_pos += len(edits[i])
            else:
                seq.append(edits[i])
                curr_pos += 1
        else:
            seq.append(chrom_seq[base_start + i])
            curr_pos += 1

    # If focal position wasn't set (shouldn't happen), default to middle
    if focal_pos_in_seq is None:
        focal_pos_in_seq = len(seq) // 2

    # Center the context window on the focal position
    half_context = context // 2
    trim_start = focal_pos_in_seq - half_context
    
    # Adjust if trim_start is negative
    if trim_start < 0:
        # Need to prepend bases from before base_start
        needed_before = -trim_start
        prepend_seq = []
        for j in range(needed_before):
            if base_start - j - 1 < 0:
                break
            prepend_seq.append(chrom_seq[base_start - j - 1])
        prepend_seq.reverse()
        seq = prepend_seq + seq
        focal_pos_in_seq += len(prepend_seq)
        trim_start = 0

    trim_end = trim_start + context + 1 # to make mutation at center with flanking base the size context // 2

    # Extend sequence if needed
    extend_pos = 0
    while len(seq) < trim_end:
        next_base_index = base_end + extend_pos
        if next_base_index >= len(chrom_seq):
            # Try to shift window left if we can't extend right
            if trim_start > 0:
                trim_start -= 1
                trim_end -= 1
            else:
                if debug:
                    return None, f"Cannot extend sequence: reached end of chromosome {chrom} at position {base_end + extend_pos}"
                return None,None
        else:
            seq.append(chrom_seq[next_base_index])
            extend_pos += 1

    # Final extraction
    if trim_end > len(seq):
        if debug:
            return None, f"Trim end {trim_end} exceeds sequence length {len(seq)}"
        return None, None

    result_seq = ''.join(seq[trim_start:trim_end])
    
    ref_base = ref_seq[half_context]
    alt_base = result_seq[half_context]
    if ref_base.upper() in {"A", "G"}:
        normalized_mutated = revcomp(result_seq)[:-1]
        normalized_reference = revcomp(ref_seq)[:-1]
        # label: complement(ref)>complement(alt)
        substitution_label = f"{comp_base(ref_base).upper()}>{comp_base(alt_base).upper()}"
    else:
        normalized_mutated = result_seq[:-1]
        normalized_reference = ref_seq[:-1]
        substitution_label = f"{ref_base.upper()}>{alt_base.upper()}"
    # reverse complement the sequence if the reference base of the substitution is not C or T,
    

    return normalized_mutated, normalized_reference
...
def get_context(v, prev_buf, next_buf, ref_genome,
    ...
    """
    TODO: 
    1. Consider the length of INDEL, SV and reflect that to the context sequence.
...
def process_input(vr, sample_name, ref_genome, tmp_dir, genome_ref38=None, liftover=False, verbose=True, context=8):
    ...
    for variant in vr:
        ...
        while len(next_buf) > 0 and (next_buf[0].chrom != variant.chrom or next_buf[0].pos < variant.pos - context):
            while len(prev_buf) > 0 and (prev_buf[0].chrom != next_buf[0].chrom or prev_buf[0].pos < next_buf[0].pos - context):
                prev_buf.popleft()
            if liftover:
...
class CustomMAFReader(VariantReader):
    """
    사용자가 제공한 특정 형식의 MAF 파일('original_annotated_selected.txt')을 파싱합니다.
    - Location: e.g., '10:122640432'
    - Uploaded_variation: e.g., '10_122640432_T/C'
    """
    def __init__(self, f, pass_only=True, type_snvs=False):
        super().__init__(f, pass_only, type_snvs)
        # 파일의 헤더 라인을 읽고 건너뜁니다.
        self.header = self.f.readline().strip().split('\t')
        try:
            # 필요한 컬럼의 인덱스를 찾습니다.
            self.loc_idx = self.header.index("Location")
            self.var_idx = self.header.index("Uploaded_variation")
            self.sample_id = "default_sample" # MAF 파일에 샘플 ID가 없으므로 기본값 사용
        except ValueError as e:
            raise IOError(f"MAF 파일 헤더에 필수 컬럼이 없습니다: {e}")
...
def test_get_context_seq():
    ...
    ref_genome = {
        "1": "ACTG" * 12 + "A"  # 49 bp total
    }

    # Case 1: SNV at position 10 (1-based), A>G
    snv = Variant(chrom="1", pos=10, ref="C", alt="G", vtype=Variant.SNV)
    seq, ref_seq = get_context_seq([], [snv], ref_genome, context=11)
    # print(ref_genome["1"][0:21],seq)
    assert seq[len(seq)//2] == "G", "SNV not correctly inserted"

    # Case 2: DEL starting at 10 (delete 3 bp: ACT)
    delvar = Variant(chrom="1", pos=10, ref="CT", alt="", vtype=Variant.DEL)
    seq, ref_seq = get_context_seq([], [delvar], ref_genome, context=15)
    
    assert len(seq) == 15, "Sequence not padded after deletion"

    # Case 3: INS at position 10 (insert GGG)
    insvar = Variant(chrom="1", pos=10, ref="A", alt="AGGG", vtype=Variant.INS)
    seq, ref_seq = get_context_seq([], [insvar], ref_genome, context=15)
    assert "GGG" in seq, "Insertion not correctly placed"
    assert len(seq) == 15, "Sequence length mismatch with insertion"


    snv1 = Variant(chrom="1", pos=10, ref="C", alt="G", vtype=Variant.SNV)
    snv2 = Variant(chrom="1", pos=12, ref="G", alt="T", vtype=Variant.SNV)
    snv3 = Variant(chrom="1", pos=32, ref="T", alt="A", vtype=Variant.SNV)
    seq1, ref_seq = get_context_seq([], [snv1, snv2, snv3], ref_genome, context=11)
    print(seq1)
    assert seq1[len(seq1)//2] == "G", "SNV1 not correctly inserted"
    assert seq1[len(seq1)//2 +2] == "T", "SNV2 not correctly inserted"

    snv1 = Variant(chrom="1", pos=10, ref="C", alt="G", vtype=Variant.SNV)
    snv2 = Variant(chrom="1", pos=12, ref="GA", alt="", vtype=Variant.DEL)
    snv3 = Variant(chrom="1", pos=32, ref="T", alt="A", vtype=Variant.SNV)
    seq1, ref_seq = get_context_seq([], [snv1, snv2, snv3], ref_genome, context=11)
    print(seq1)
    assert seq1[len(seq1)//2] == "G", "SNV1 not correctly inserted"
    assert seq1[len(seq1)//2 +2] == "C", "Deletion not correctly inserted"


    print("✅ All tests passed.")

if __name__ == "__main__":
    test_get_context_seq()
